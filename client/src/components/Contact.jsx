...
class Contact extends Component {
	...
	async componentDidMount() {
		...
		let contact = user.job.filter(job => job.contactName && job.phone || job.companyName && job.phone)
		this.setState({ jobs: contact })
	}
	render() {
		...
		return (
			<div>
				...
				<br></br>
			<SnackbarContent className={classes.snackbar} message='Contacts' variant='h6' />
			{this.state.jobs.length === 0 && <Typography className={classes.msg}>You don't have any contacts :(</Typography>}
			<Paper className={classes.root}>
